...
public sealed class AiLiteKnowledgeService
{
    ...
    public string Answer(string query)
    {
        if (string.IsNullOrWhiteSpace(query))
            return "Ask about events, results, configuration, local AI, or NPC simulation with .ai <text>.";
        ...
        return best == null || best.Score <= 0
            ? "I could not match that safely. Ask with .ai <text> about events, results, configuration, local AI, or NPC simulation."
            : best.Entry.Answer;
    }
