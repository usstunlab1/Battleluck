...
export default function App() {
  ...
  const [layerBosses, setLayerBosses] = useState(true);
  const [layerResources, setLayerResources] = useState(true);
  const [layerWaygates, setLayerWaygates] = useState(true);
  const [layerPlots, setLayerPlots] = useState(true);
        ...
        <main className="flex-1 h-full relative overflow-hidden bg-slate-950">
          {embedMode ? (
            <div className="w-full h-full relative flex flex-col bg-zinc-950">
              <div className="bg-red-950/80 border-b border-red-800/80 px-4 py-1.5 flex items-center justify-between text-xs text-red-200 font-serif">
                <span className="font-bold flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full bg-amber-400 animate-pulse" />
                  MAPGENIE REFERENCE (Read-Only External Reference View)
                </span>
                <button 
                  onClick={() => setEmbedMode(false)}
                  className="underline text-red-300 hover:text-white font-mono"
                >
                  Switch to Native Editable Admin MapCanvas
                </button>
              </div>
              <iframe 
                src="https://mapgenie.io/v-rising/maps/vardoran?embed=light" 
                className="w-full h-full border-0"
                title="V Rising MapGenie Vardoran Map"
