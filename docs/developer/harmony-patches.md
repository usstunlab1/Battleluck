...
Code examples adapted from [KindredCommands](https://github.com/Odjit/KindredCommands) (AGPL-3.0).
...
ECS systems store their `EntityQuery` objects as private fields. You can access them directly on `__instance` by matching the field name exactly as it appears in ILSpy.

```csharp
[HarmonyPatch(typeof(UpdateBuffsBuffer_Destroy), nameof(UpdateBuffsBuffer_Destroy.OnUpdate))]
internal static class UpdateBuffsBuffer_DestroyPatch
{
    public static void Prefix(UpdateBuffsBuffer_Destroy __instance)
    {
        ...
        foreach (var entity in entities)
        {
            ...
            if (attach.Parent != Entity.Null)
                Core.PlayerStateService.HandleShroudRemoval(attach.Parent);
        }
        entities.Dispose();
    }
}
...
For a complete reference of query field names and their component filters, see the [Entity Query Descriptions](https://wiki.vrisingmods.com/dev/query-descriptions.html).
...
The preferred approach is to destroy the relevant event entity before the system processes it:
    ...
    public static void Prefix(BloodAltarSystem_StartTrackVBloodUnit_System_V2 __instance)
    {
        ...
        try
        {
            foreach (var entity in entities)
            {
                ...
                if (Core.BossService.IsBossLocked(huntTarget))
                {
...
**Guard against uninitialized Core.** If a system fires before your core services are ready, a null reference will crash the server:
...
**Never hold onto NativeArray across frames.** It is undefined behavior and will crash:

```csharp
var entities = __instance._Query.ToEntityArray(Allocator.Temp);
// Must dispose before leaving the method
entities.Dispose();
```

**Don't call `UnpatchAll()` in Postfix.** Unpatching during execution of a patch causes undefined behavior. Use targeted `Unpatch()` instead.
